...
**TEST section — pinned scroll stepper, `#scroll-test` (`index.html`, `.scroll-stepper*` in `css/style.css`, `js/main.js`).** Sitting under the hero purely to try out — no nav link, no section number, not part of the real IA yet. Inspired by verostudio.com's pinned-reveal hero (inspected directly: a `.mask` element driven by a `--mask-progress` custom property feeding a `clip-path` `polygon()`/`inset()`, recomputed as the user scrolls through a tall pinned track — not a drag-driven slider despite Awwwards' "interactive slider" label). Built the same way by hand here: a `600vh` track holds a `position: sticky` viewport-height stage; 5 full-bleed Auko photos (`object-fit: cover` — full-bleed hero treatment is the one place on the site that's exempt from the no-cropping rule, same as the original hero photo was) sit stacked in DOM order, each wiping in from the right over the one beneath as scroll passes its slot (`clip-path: inset(0 0 0 ${(1-p)*100}%)`, `p` from a `scroll`+`rAF`-throttled progress read of the track's `getBoundingClientRect()` — no scroll-jacking, just CSS `sticky` plus reading position). Frame 0 is the resting base layer and is never clipped. A bottom scrim plus a big `.h-display` headline and `.lead` description swap per step, currently sourced from the real Services copy (title/description pulled verbatim from the Services section) rather than new copy, at the user's request for "big font." `prefers-reduced-motion` collapses the whole thing to a static ~70vh single frame with no pin and no track height, matching how the rest of the site degrades.
